...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
	...
	// action sheet used to choose report location where app has been granted access to location services
	if (actionSheet.tag == 2) {
		...
		if (buttonIndex == 0) {
			...
			if ([self isGPSLocationAccurateEnough:DataRepository.sharedInstance.proposedLocation] && [[DataRepository sharedInstance] locationIsInCity:DataRepository.sharedInstance.proposedLocation])
			{
				[[self setReportLocationButtonLabel] setText:@"Change Report Location"];
			}
			else 
	...
	// action sheet used to choose report location where app does not have access to location services
...
- (void)sendReportToTrackIT {
	...
	if (reportTypeIndex >= 0) {
		CRMReportDefinition *instance = [[globalData crmReportDefinitionArray] objectAtIndex:reportTypeIndex];
		if (instance != nil) {
...
- (void)getGPSLocationTimerFired {
	...
	if ([self isGPSLocationAccurateEnough:globalData.proposedLocation] == YES) {
		...
		if ([globalData locationIsInCity:globalData.proposedLocation] == YES) {
			...
			[[self setReportLocationButtonLabel] setText:@"Change Report Location"];
			//[[self reportCommentsTextView] setText:[NSString stringWithFormat:@"Location horizontal accuracy = ± %0.1f meters", globalData.proposedLocation.horizontalAccuracy]];
			return;
		}
		else {
